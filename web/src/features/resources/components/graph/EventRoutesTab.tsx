...
export function EventRoutesTab({
  ...
  // Get all sink names
  const allSinkNames = [
    ...(eventSinks.kafka ? eventSinks.kafka.map((s) => s.name) : []),
    ...(eventSinks.kusto ? eventSinks.kusto.map((s) => s.name) : []),
    ...(eventSinks.mqtt ? eventSinks.mqtt.map((s) => s.name) : []),
    ...(eventSinks.webhook ? eventSinks.webhook.map((s) => s.name) : []),
  ].filter(Boolean);

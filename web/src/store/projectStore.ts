...
interface ProjectState {
  ...
  updateProject: (projectId: string, data: Partial<{ name: string; description: string }>) => Promise<Project | null>;
...
fetchProjectById: async (projectId: string) => {
    ...
    try {
        ...
        const errorMsg = axios.isAxiosError(err)
            ? err.response?.data?.error || err.message
            : (err instanceof Error ? err.message : `Failed to fetch project ${projectId}`);
        toast.error(errorMsg);
        set({ error: errorMsg, isLoadingDetail: false, currentProject: null });
    }
},
...
updateProject: async (projectId, data) => {
    ...
    try {
        ...
        set((state) => ({
            projects: state.projects.map((p) => (p.project_id === projectId ? updatedProject : p)),
            currentProject: state.currentProject?.project_id === projectId ? updatedProject : state.currentProject,
        }));
